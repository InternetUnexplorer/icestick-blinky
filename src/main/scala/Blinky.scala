...
// We don't need a reset, so we make `Blinky` a `RawModule` and define the clock
// explicitly to avoid an implicit reset.
...
object Blinky extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new Blinky, args)
}
